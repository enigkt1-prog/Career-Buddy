...
| **C — Classify + Layer-3 backfill session** (round 9, 2026-05-10 → 11) | `backend/career_buddy_scraper/cli/classify_subcat*.py`, `backend/career_buddy_scraper/cli/layer3_backfill*.py`, `backend/audit/classify_subcat-*.csv`, `backend/audit/layer3_backfill_*.csv` | **Task 1 DONE** — `other`-bucket fully drained (8999 → 0). Final sub-cat distribution: engineering 2415, sales 2125, operations 931, data-science 692, customer-success 662, finance-legal 609, marketing 500, product 395, recruiting-people 366, other-misc 158, design 146. The 6 fitting buckets (bd 497, strategy 231, bizops 178, founders-associate 43, investment-analyst 25, chief-of-staff 7) never touched. NULL=2524 are NEW jobs from D's mid-flight scraper writes (re-run later picks them up). **Task 2 in flight** — `level` backfill cycling now (7457 NULL → ~40% will stay NULL by design = conservative). Then years → city → visa → salary. Read-mostly LLM batch lane; no `src/**`, no migrations, no edge functions touched. |
